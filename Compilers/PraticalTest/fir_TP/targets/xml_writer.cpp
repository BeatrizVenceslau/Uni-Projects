...
//---------------------------------------------------------------------------
/*Teste Pratico*/
void fir::xml_writer::do_apply_node(fir::apply_node * const node, int lvl) {
  openTag(node, lvl);
  ...
  os() << std::string(lvl, ' ') << "<in function '" << node->func() << "'>" << std::endl;
...
void fir::xml_writer::do_while_finally_node(fir::while_finally_node *const node, int lvl) {
  ...
  openTag("finallyBlock", lvl + 2);
  node->finallyBlock()->accept(this, lvl + 4);
  closeTag("finallyBlock", lvl + 2);
...
void fir::xml_writer::do_variable_decl_node(fir::variable_decl_node *const node, int lvl) {
  ...
  node->qualifier() << "' name='" << node->name() << "'>" << std::endl; 
...
void fir::xml_writer::do_function_definition_node(fir::function_definition_node *const node, int lvl) {
  ...
  openTag("return_value",lvl);
  if(node->return_value()){
    node->return_value()->accept(this,lvl+4);
  }
  closeTag("return_value",lvl);
...
void fir::xml_writer::do_leave_node(fir::leave_node *const node, int lvl) {
  os() << std::string(lvl, ' ') << "<" << node->label() << " ncicle='" << 
  std::to_string(node->ncicle()) << "'>" << std::endl; 
  closeTag(node,lvl);
}
...
void fir::xml_writer::do_index_node(fir::index_node *const node, int lvl) {
  os() << std::string(lvl,' ') << "<" << node->label() << " pointer='" << node->pointer() << "'>" << std::endl;
