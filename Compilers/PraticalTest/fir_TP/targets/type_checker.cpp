...
void fir::type_checker::do_apply_node(fir::apply_node *const node, int lvl) {
  ...
  if (!node->low()->is_typed(cdk::TYPE_INT)) 
    throw std::string("expected integer expression for apply low index");

  node->high()->accept(this, lvl + 4);
  if (!node->high()->is_typed(cdk::TYPE_INT)) 
    throw std::string("expected integer expression for apply high index");

  node->vector()->accept(this, lvl + 4);
  if (!node->vector()->is_typed(cdk::TYPE_POINTER)) 
    throw std::string("expected pointer expression for apply vector");

  const std::string &vector = node->func();
  auto svector = _symtab.find(vector);
  if (svector == nullptr) 
    throw std::string("vector '" + vector + "' is undeclared.");

  const std::string &func = node->func();
  auto sfunc = _symtab.find(func);
  if (sfunc == nullptr) 
    throw std::string("function '" + func + "' is undeclared.");
  else if (sfunc->number_of_args() != 1)
    throw std::string("function '" + func + "' has to many arguments, only one argument expected.");

  if(sfunc->arg_type() != svector->arg_type())
    throw std::string("function '" + func + "' and vector '" + vector + "' have incompatible types.");
}
